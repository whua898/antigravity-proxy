...
$dllPattern = if ($Config -eq "Debug") { "winmm*.dll" } else { "winmm.dll" }
...
通过劫持 winmm.dll，可以透明地将目标进程的网络流量重定向到代理服务器。
...
5. **劫持方案变更**: 从 version.dll 切换为 winmm.dll，大幅提升对 JVM/PyCharm 等复杂应用的兼容性
